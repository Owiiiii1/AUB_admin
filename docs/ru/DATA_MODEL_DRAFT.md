...
`attendance_records`: **Student + ScheduledLesson**; `status` `present`/`absent`/`excused`; unique `(scheduled_lesson_id, student_id)`; `marked_by` nullable `nullOnDelete`. Unmarked = нет строки. **Нет `AttendanceRecord` ≠ `absent`**. Teacher write + история месяца Student/Parent. См. [ATTENDANCE.md](ATTENDANCE.md).  
...
| **Student Attendance** | Teacher write + история месяца Student/Parent (`attendance_records`). Текущий roster для отметки. Нет `AttendanceRecord` ≠ `absent`. Финализация OPEN | Teacher write + история реализованы / финализация OPEN |
