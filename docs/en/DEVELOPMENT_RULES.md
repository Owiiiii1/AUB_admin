...
6. Mobile work may proceed in parallel; **features** need an API contract. Auth/`/me` contract: [API.md](API.md). Next: Flutter Authentication Foundation.
...
- `.env` and `.env.testing` are not committed (see `.env.testing.example`)
...
- Safe check of the test schema: `php artisan migrate:status --env=testing` (uses `.env.testing`; Laravel `--env=testing` loads that file).
