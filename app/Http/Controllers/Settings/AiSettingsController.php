...
class AiSettingsController extends Controller
{
     ...
     */
    public function providersForPage(): array
    {
    ...
    public function saveKey(Request $request, string $provider): RedirectResponse
    {
        ...
        $this->activityLogger->log(
            ...
            'ai_provider',
            $setting->id,
            $setting->label ?: $setting->provider,
            null,
            ['changes' => ['api_key' => ['***', '***']]],
        );

        return $this->redirectToAiTab()->with('success', 'API key saved.');
    }
    ...
    public function check(Request $request, string $provider): RedirectResponse
    {
        ...
        if (! filled($setting->api_key)) {
            return $this->redirectToAiTab()->withErrors(['ai' => 'Save API key before checking connection.']);
        }
        ...
        try {
            ...
            $this->activityLogger->log(
                ...
                $setting->label ?: $setting->provider,
                null,
                ['connected' => true, 'models_count' => count($models)],
            );

            return $this->redirectToAiTab()->with('success', 'Connection checked. Models loaded.');
        } catch (Throwable $e) {
            $setting->fill([
                'is_connected' => false,
                'available_models' => null,
            ...
            return $this->redirectToAiTab()->withErrors(['ai' => $e->getMessage()]);
        }
    }
    ...
    public function activate(Request $request, string $provider): RedirectResponse
    {
        ...
        if (! in_array($validated['model'], $available, true)) {
            return $this->redirectToAiTab()->withErrors([
        ...
        return $this->redirectToAiTab()->with('success', 'Provider activated.');
    }
    ...
    public function deactivate(Request $request): RedirectResponse
    {
        ...
        return $this->redirectToAiTab()->with('success', 'AI provider deactivated.');
    }

    private function redirectToAiTab(): RedirectResponse
    {
